	...
	*/
	String.prototype.splitCSV = function(sep) {
		for (var thisCSV = this.split(sep = sep || ","), x = thisCSV.length - 1, tl; x >= 0; x--) {
			if (thisCSV[x].replace(/"\s+$/, '"').charAt(thisCSV[x].length) == '"') {
				if ((tl = thisCSV[x].replace(/^\s+"/, '"')).length > 1 && tl.charAt(0) == '"') {
					...
					thisCSV.splice(x - 1, 2, [thisCSV[x - 1], thisCSV[x]].join(sep));
                        ...
                        thisCSV[x] = thisCSV[x].replace(/^"/,"").replace(/"$/,"").replace(/^'/,"").replace(/'$/,"");
		} return thisCSV;
	};
	...
	$.fn.CSVToTable = function(csvFile, options) {
		...
		return this.each(function() {
			...
			$.get(csvFile, function(data) {
				var schedValue = ["HOME","AWAY","DATE","TIME","FIELD"];
				...
				$.each(lines, function(lineCount, line) {
					if ((lineCount == 0) && (typeof(options.headers) != 'undefined')) {
						...
						$.each(headers, function(headerCount, header) {
							tableHTML += '<th class="' + options.thClass + '">' + header.toUpperCase() + '</th>';
						});
					...
					if ((lineCount == options.startLine) && (typeof(options.headers) == 'undefined')) {
						...
						tableHTML += '<thead class="' + options.theadClass + '"><tr class="' + options.trClass + '">';
						$.each(headers, function(headerCount, header) {
							tableHTML += '<th class="' + options.thClass + '"><select name="row[' + lineCount + '][column' + headerCount + ']" id="row[' + lineCount + '][column' + headerCount + ']">';
							if( header.toUpperCase() in oc(schedValue) ) {
							tableHTML += '<option select="selected" value=' + header.toUpperCase() + '>' + header.toUpperCase() + '</option>';
							for (var i = 0, schedule; schedule = schedValue[i]; i++) {
                                                          tableHTML += '<option value=' + schedule + '>' + schedule +'</option>';
                                                          };
							}
							else {
							tableHTML += '<option select="selected" value="Unknown">Unknown</option>';
							for (var i = 0, schedule; schedule = schedValue[i]; i++) {
							  tableHTML += '<option value=' + schedule + '>' + schedule +'</option>';
							  };
							};
						...
						if (items.length > 1) {
							...
							$.each(items, function(itemCount, item) {
								tableHTML += '<td><input type="text" name="row[' + lineCount + '][column' + itemCount + ']" id="row[' + lineCount + '][column' + itemCount + ']" value="' + item + '"></td>';
							});
							tableHTML += '</tr>';
							tblRow++;
						}
